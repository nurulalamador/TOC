...
function nfaToDfa(NFA, StateOrder, Closures) {
    var DFA = {};
    var stateOrders = [];
    ...
    var knownStates = [Closures[StateOrder[0]]]
    var unknownStates = [];
    ...
    for(var i=0; i<symbols.length; i++) {
        transitionArray = []; 
        subStates = knownStates[0].split(",");
        

        subStates.forEach(state => {
            subSubStates = Closures[state].split(",");

            subSubStates.forEach(element => {
                subTransitions = NFA[element][symbols[i]].split(",");

                subTransitions.forEach(subTransition => {
                    try {
                        closuresSubTransitions = Closures[subTransition].split(",");
                        closuresSubTransitions.forEach(closuresSubTransition => {
                            if(!transitionArray.includes(closuresSubTransition) && closuresSubTransition !='') {
                                transitionArray.push(closuresSubTransition);
                            } 
                        });
                    }
                    catch {}
        ...
        transition = transitionArray.sort().toString();
    ...
    currentKnownState = knownStates[0].split(",").sort().toString();
    DFA[currentKnownState] = currentTransition;
    if(!stateOrders.includes(currentKnownState)){
        stateOrders.push(currentKnownState);
    }
    ...
    while (unknownStates[k]) {
        ...
        for(var j=0; j<symbols.length; j++) {
            ...
            unknownStateArray.forEach(state => {
                if(state == "qØ") {
                    transitionArray.push("qØ");

                }
                else {
                    subSubStates = Closures[state].split(",");
                    subSubStates.forEach(element => {
                        subTransitions = NFA[element][symbols[j]].split(",");
            ...
            newCreatedState = transitionArray.sort().toString();
...
function generateNfa(NFA,StateOrder,Closures) {
    ...
    closureKeys = Object.keys(Closures);
    closureKeys.forEach(keys => {
        Closures[keys] = Closures[keys].split(",").sort();
        Closures[keys] = Closures[keys].filter(function(e) { return e != keys });
    })
    closureKeys.forEach(keys => {
        Closures[keys].forEach(el => {
            Closures[el].forEach(elm => {
                if(Closures[keys].includes(elm)) {
                    Closures[keys] = Closures[keys].filter(function(e) { return e != elm });
                }
            });
        });
    });
    ...
    closureKeys.forEach(keys => {
        Closures[keys].forEach(element => {
            preRegExp = `"${keys}" -> "${element}" \\[label = "(.*)"];`;
            regExp = new RegExp(preRegExp, "g");

            if(vizString.match(regExp) != null) {
                vizString = vizString.replace(regExp, `"${keys}" -> "${element}" [label = "$1, ɛ"];`);
            }
            else {
                vizString += ('"'+keys+'" -> "'+element+'" [label = "ɛ"];\n');
            }
        });
    });
...
document.getElementById("nextStep").onclick = function() {
    ...
    document.getElementById("closureList").innerHTML = "";
    ...
    for(var i=0; i<state.length; i++) {
        ...
        document.getElementById("closureList").insertAdjacentHTML("beforeend", `<div class="addState" onclick="addTransition('${state[i].value}')">${state[i].value}</div>`);
    ...
    var eClosuresTable = document.createElement("table");
    eClosuresTable.setAttribute("id", "eClosuresHtmlTable");
    for(var i=0; i<state.length; i++) {
        tr = document.createElement("tr");
        th = document.createElement("th");
        th.innerHTML = "ɛ-Closure ("+state[i].value+")";
        tr.append(th);

        td = document.createElement("td");
        td.setAttribute("class","hide");
        td.innerHTML = state[i].value;
        tr.append(td);

        td = document.createElement("td");
        td.setAttribute("class","closuresSet");
        td.setAttribute("onclick","openClosureMenu(this)");
        td.innerHTML = state[i].value;
        tr.append(td);

        eClosuresTable.append(tr);
    }
    
    document.getElementById("eClosuresTable").innerHTML = "";
    document.getElementById("eClosuresTable").append(eClosuresTable);
...
function openClosureMenu(element){
    document.getElementById("closureListBox").classList.remove("hide");
    currentElement = element;
}
...
function printNfaPreview() {
    ...
    var closuresTable = document.getElementById("eClosuresHtmlTable");
    var Closures = getClosureJson(closuresTable);

    var vizString = generateNfa(NFA,NFAOrder,Closures);
...
function addTransition(state) {
    ...
    document.getElementById("closureListBox").classList.add("hide");
...
document.getElementById("closeClosureListBox").onclick = function() {
    document.getElementById("closureListBox").classList.add("hide"); 
}
...
document.getElementById("removeClosure").onclick = function() {
    document.getElementById("closureListBox").classList.add("hide"); 
    var firstElement = currentElement.innerHTML.split(",")[0];
    currentElement.innerHTML = firstElement;
...
document.getElementById("convertToDfa").onclick = function() {
    ...
    var nfaToDfaData = nfaToDfa(NFA, NFAOrder, Closures);
