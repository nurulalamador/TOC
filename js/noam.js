...
(function (noam) {
  ...
  noam.fsm.printHtmlTable = function(fsm) {
    ...
    htmlString.push("<table border='1' class='table text-white'>");
    htmlString.push("  <tr>");
    
    //head states
    htmlString.push("    <th> </th>");
    ...
    for (i=0; i<tableRows.length; i++) {
      ...
      for (j=0; j<tableRows[i].length-1; j++) {
        if(j==0) {
          htmlString.push("    <td>"+((i==0 && j==0) ? '→ ' : '' )+""+(((tableRows[i][tableRows[i].length-1].text).toString() == '1') ? "* " : ""  )+"q" + tableRows[i][j].text + "</td>");
        }
        else {
          htmlString.push("    <td>q" + tableRows[i][j].text + "</td>");
        }
      }
  ...
  noam.fsm.printDotFormat = function(fsm) {
    ...
    for (i=0; i<fsm.acceptingStates.length; i++) {
      accStates.push("q"+fsm.acceptingStates[i].toString());
    }
    ...
    initStateArrow.push("q"+fsm.initialState.toString());
    ...
    for (i=0; i<newTransitions.length; i++) {
      if (noam.util.areEquivalent(newTransitions[i].toStates[0], fsm.initialState)) {
        trans = [" "];
        trans.push("q"+newTransitions[i].toStates[0].toString());
        trans.push("->");
        trans.push("q"+newTransitions[i].fromState.toString());
        trans.push("[");
        trans.push("label =");
